...
while ($j > 0) {
	my @listOfQuestions = keys %set;
	my $question = $listOfQuestions[rand($numberOfQuestions)];
...
# the if statement. If it's a classic MCQ, it falls in the else part.
	print colored("\n$question\n", 'bold yellow');
	if ($rightAnswer =~ /@@/) {
		$rightAnswer = substr($rightAnswer, 2);
		my $userAnswer = <STDIN>;
		chomp $userAnswer;
		if ($userAnswer eq $rightAnswer) {
			...
			print "False\n";
			print "The answer was: $rightAnswer\n";
			print color 'reset';
		}
	} else {
		my $letter = 'A';
